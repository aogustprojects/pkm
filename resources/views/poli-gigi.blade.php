...
<div class="min-h-full">
    ...
    @php
        ...
        $endTime = $now->copy()->setTime(16, 0); // Updated to 16:00
    ...
    <main>
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <div class="border p-2 rounded-lg">
                <!-- Debug Statements -->
                {{-- <p class="text-center text-sm bg-white p-2 rounded-lg text-blue-500 shadow-lg">
                    Debug: maxRegistrationsPerDay = {{ $maxRegistrationsPerDay }}, isFormOpen = {{ $isFormOpen ? 'true' : 'false' }}
                </p> --}}

                @auth
                    {{-- <p class="text-center text-sm bg-white p-2 rounded-lg text-green-500 shadow-lg">
                        User is authenticated! User ID: {{ auth()->id() }}
                    </p> --}}
                    <p class="text-center text-sm bg-white p-2 rounded-lg text-red-500 shadow-lg">
                    ...
                    <form action="{{ route('toggle-form-status') }}" method="POST" class="mb-4">
                        @csrf
                        <label for="is_form_open" class="block text-sm font-medium text-gray-900">Form Status:</label>
                        <select name="is_form_open" id="is_form_open" class="mt-2 block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 outline-gray-300">
                            <option value="1" {{ $isFormOpen ? 'selected' : '' }}>Open</option>
                            <option value="0" {{ !$isFormOpen ? 'selected' : '' }}>Closed</option>
                        </select>
                        <button type="submit" class="mt-2 bg-blue-500 text-white px-4 py-2 rounded-md">Update Status</button>
                    </form>
