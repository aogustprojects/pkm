                ...
                <div class="relative flex h-full flex-col overflow-hidden rounded-xl bg-white">
                    <div class="px-8 py-8 sm:px-10 sm:pt-10">
                        ...
                        <div class="flex flex-col lg:flex-row justify-between items-center mb-6 gap-4">
                            <p class="text-2xl font-semibold tracking-tight text-gray-900 max-lg:text-center">List Surat Keluar</p>
                            <form action="{{ route('arsip_surat_keluar.index') }}" method="GET" class="flex items-center w-full max-w-md gap-2">
                            ...
                            <a href="/dashboard/arsip_surat_keluar/create" class="bg-indigo-600 rounded-lg px-4 py-2 text-sm text-white font-medium hover:bg-indigo-700 transition-all duration-200 shadow-sm hover:shadow-md">
                                Tambah Surat Keluar
                            </a>
                        </div>
                            ...
                            <table class="w-full text-sm text-left text-gray-600">
                                <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-4 text-center">Dikirim Ke</th>
                                        <th scope="col" class="px-6 py-4 text-center">File</th>
                                        <th scope="col" class="px-6 py-4 text-center">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse ($arsipSuratKeluar as $surat)
                                        <tr class="bg-white border-b border-gray-200 hover:bg-gray-50 transition-colors duration-150">
                                            ...
                                            <td class="px-6 py-4 text-center">{{ $surat->dikirim_kepada }}</td>
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                <div class="flex justify-center items-center space-x-2">
                                                    <a href="{{ route('arsip_surat_keluar.show', $surat->id) }}" class="p-2 bg-gray-100 hover:bg-blue-600 text-blue-500 hover:text-white rounded-lg border border-gray-200 transition-all duration-200">
                                                        <i class="fa-solid fa-eye"></i>
                                                    </a>
                                                    <a href="/dashboard/arsip_surat_keluar/{{ $surat->id }}/edit" class="p-2 bg-gray-100 hover:bg-yellow-600 text-yellow-500 hover:text-white rounded-lg border border-gray-200 transition-all duration-200">
                                                        <i class="fa-solid fa-pen"></i>
                                                    </a>
                                                    <form action="{{ route('arsip_surat_keluar.destroy', $surat->id) }}" method="POST" class="inline-flex items-center">
                                                        @csrf
                                                        @method('DELETE')
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="8" class="text-center py-6 text-gray-500">Tidak ada data tersedia</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
